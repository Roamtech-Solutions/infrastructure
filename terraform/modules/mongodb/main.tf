...
resource "google_secret_manager_secret" "users" {
  ...
  secret_id = "${var.name}-mongodb-${each.key}"
  replication {
    auto {}
  }
}
...
resource "google_service_account" "mongodb" {
  account_id   = "${var.name}-mongodb"
  display_name = "${var.name}-mongodb"
  project      = var.project_id
}
...
/* TODO: K8s job to run which connects to mongodb and runs the user creation script */
